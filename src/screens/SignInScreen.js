...
const redirect = P.location.search
 ? P.location.search.split('=')[1] //Everything after = sign
 : '/';
console.log('redirect', P.location.search.split('=')[1]) //signin?redirect=[shipping] (We Just want Shipping)
...
const { userInfo, loading, error } = userSignin;
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    dispatch(signin(email, password)); //if email and signin match then match the correct database
}
